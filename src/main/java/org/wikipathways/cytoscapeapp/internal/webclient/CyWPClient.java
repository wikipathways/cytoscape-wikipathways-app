...
public class CyWPClient extends AbstractWebServiceGUIClient implements NetworkImportWebServiceClient, SearchWebServiceClient {
  ...
  final JTable resultsTable = new JTable(new DefaultTableModel(RESULTS_TABLE_COLUMN_NAMES, 0));
  WPClient client;
  ...
  class PerformSearch extends InterruptableTask {
    public void interruptableRun(TaskMonitor monitor) throws Exception {
      ...
      final DefaultTableModel dm = (DefaultTableModel) resultsTable.getModel();
      ...
      for (final WPClient.PathwayRef pathwayRef : pathwayRefs) {
        dm.addRow(new Object[] { pathwayRef.getName(), pathwayRef.getSpecies() });
      }
